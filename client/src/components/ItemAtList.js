...
import {DETAIL_POST__TITLE__POSTID} from '../constants/path'
...
const ItemAnList = ({
  ...
  const OnclickNavigateDetail = ()=>{
    Navigate(DETAIL_POST__TITLE__POSTID)
  }
  ...
  return (
    star >= 4 ? <div className="mx-[-20px] item flex items-start py-4 px-5 border-t border-[#E13427] bg-[#fff9f3]">
      {images.length > 4 ? <div className="relative w-2/5">
        <Link to={`chi-tiet/${normalizedString(title)}/${id}`} 
      ...
      <div className="flex justify-between gap-4 w-full">
        <div className="flex items-start">
          <span className="text-[#e13427] font-bold text-base cursor-pointer hover:underline uppercase line-clamp-2">
            ...
            <span className="ml-[3px]">{title && title}</span>
          </span>
        </div>
  ...
  </div> : <div className="item flex items-start py-4 border-t bg-white border-[#ea2e9d]">
